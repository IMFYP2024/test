<template>
  ...
  const startPosition = { lat: userPosition.lat, lng: userPosition.lng };
  const endPosition = { lat: coord[0], lng: coord[1] };
  const distance = haversineDistance(startPosition.lat, startPosition.lng, endPosition.lat, endPosition.lng);
  ...
  while (accumulatedDistance < distance) {
    ...
    const waypoint = {
      lat: startPosition.lat + ratio * (endPosition.lat - startPosition.lat),
      lng: startPosition.lng + ratio * (endPosition.lng - startPosition.lng)
    };

    
    coordinates.value.push([waypoint.lat, waypoint.lng]);

    
    const waypointEntity = document.createElement('a-entity');
    waypointEntity.setAttribute('geometry', {
        primitive: 'box',
        width: 0.2, 
        height: 0.2, 
        depth: 0.2   
    });
    waypointEntity.setAttribute('gps-new-entity-place', `${waypoint.lng} 0 ${waypoint.lat}`);
    waypointEntity.setAttribute('scale', '4 4 4');
    waypointEntity.setAttribute('visible', 'true');
    waypointEntity.setAttribute('look-at', '[camera]');
    scene.appendChild(waypointEntity);
